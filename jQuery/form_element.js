$().ready(function () {
  $(".email-validation-button").on("click", function (event) {
    ...
    // 만약 입력값이 5자리 미만이라면 "이메일은 5자리 이상으로 입력해주세요." 경고창을 노출시킨다.
  ...
  $(".jobs-validation-button").on("click", function (event) {
    ...
    // "#jobs"인 것의 value를 가져와서 alert으로 노출한다.
    var jobId = $("#jobs").val();
  ...
  $("#checked-all").on("change", function () {
    ...
    // 체크가 되었다면 true 체크가 안되었다면 false

    // 2. 전체 선택이 체크되어있다면 나머지 체크박스들도 모두 체크한다.
    // 3. 전체 선택이 체크해제되어있다면 나머지 체크박스들도 모두 체크해제한다.
  ...
  $("input[type='checkbox'][name='favorate-genre']").on("change", function () {
    ...
    // 3. 1 과 2의 결과가 같다면 전체선택을 체크한다.
    // 4. 1 과 2의 결과가 다르다면 전체선택을 체크해제한다.
